...
class QueenBoard:
    ...
    def placeInNextRow(self, column):
        self.columns.append(column)
 
    def removeInCurrRow(self):
        return self.columns.pop()
 
    # to check the next row of this column is safe
    def nextRowCheck(self, column):
        # index of next row
...
def solve_queen(size):
    ...
    while True:
        ...
        while column < size:
            if board.nextRowCheck(column):
                board.placeInNextRow(column)
                row += 1
                column = 0
                break
            else:
        ...
        if (column == size or row == size):
            ...
            if row == size:
                ...
                board.removeInCurrRow()
                row -= 1
 
            # now backtrack
            try:
                prev_column = board.removeInCurrRow()
            except IndexError:
                # all queens removed
